...
file.copy("C:/Users/SPoudel/Ventura Foods/SC Analytics Team - General/Stan Report Files/File Repository/IQR Automation/RM/Weekly Run Files/2025/02.04.2025/Raw Material Inventory Health (IQR) NEW TEMPLATE - 02.04.2025.xlsx",
          "C:/Users/SPoudel/Ventura Foods/SC Analytics Team - General/Stan Report Files/File Repository/IQR Automation/RM/Weekly Run Files/2025/02.11.2025/Raw Material Inventory Health (IQR) NEW TEMPLATE - 02.11.2025.xlsx",
          overwrite = TRUE)
...
consumption_data <- read_excel("S:/Supply Chain Projects/Linda Liang/reference files/Raw Material Monthly Consumption - 2025.01.09.xlsx")
...
rm_data <- read_excel("C:/Users/SPoudel/Ventura Foods/SC Analytics Team - General/Stan Report Files/File Repository/IQR Automation/RM/Weekly Run Files/2025/02.11.2025/Raw Material Inventory Health (IQR) NEW TEMPLATE - 02.11.2025.xlsx", 
...
rm_data %>% 
  ...
  dplyr::mutate(item = as.character(item),
                planner = as.character(planner)) -> rm_data
...
# BoM_dep_demand ----
bom_dep_demand <- read_excel("C:/Users/SPoudel/Ventura Foods/SC Analytics Team - General/Stan Report Files/BOM/02.11.2025/Bill of Material_02042025.xlsx",
                             sheet = "Sheet1")
...
ss_optimization <- read_excel("C:/Users/SPoudel/Ventura Foods/SC Analytics Team - General/Stan Report Files/File Repository/Lookup Datasets/Weekly Dataset/02.11.2025/SS Optimization by Location - Raw Material LIVE.xlsx",
                              sheet = "Sheet1")
...
ss_optimization %>% 
  janitor::clean_names() %>% 
  readr::type_convert() %>% 
  dplyr::rename(eoq_adjusted = eoq_adjusted_for_shelf_life) -> ss_optimization
...
# Lot At Risk $
rm_at_risk_file <- read_excel("C:/Users/SPoudel/Ventura Foods/SC Analytics Team - General/Stan Report Files/File Repository/IQR Automation/RM/Weekly Run Files/2025/02.11.2025/Raw Material Inventory Health (IQR) NEW TEMPLATE - 02.11.2025.xlsx",
...
bom <- read_excel("C:/Users/SPoudel/Ventura Foods/SC Analytics Team - General/Stan Report Files/File Repository/Lookup Datasets/Weekly Dataset/02.11.2025/JDE BoM 02.11.2025.xlsx",
                  sheet = "BoM")
...
bom %>% 
  data.frame() %>% 
  rename(loc_sku = comp_ref) %>% 
  select(loc_sku, supplier_number, supplier_name) %>% 
  mutate(loc_sku = gsub("-", "_", loc_sku)) %>% 
  distinct(loc_sku, .keep_all = TRUE) -> bom_supplier



rm_data %>% 
  dplyr::select(-supplier_number, -supplier_name) %>% 
  dplyr::left_join(bom_supplier, by = "loc_sku") %>% 
  dplyr::mutate(supplier_number = ifelse(is.na(supplier_number), "NA", supplier_number),
...
rm_data %>%
  left_join(exception_report_dou %>%
              rename(loc_sku = ref) %>% 
              filter(loc_sku %in% rm_data$loc_sku[which(rm_data$planner == "DNRR")]), by = "loc_sku") %>%
  dplyr::mutate(planner = ifelse(planner.x == "DNRR", planner.y, planner.x)) %>% 
  dplyr::select(-planner.x, -planner.y, -planner_name) %>%
  dplyr::left_join(planner_adress %>% rename(planner_name = alpha_name) %>% select(planner, planner_name)) %>% 
  ...
  dplyr::mutate(planner = ifelse(is.na(planner) | planner == 0, "DNRR", planner),
                planner_name = ifelse(is.na(planner_name) | planner_name == 0, "DNRR", planner_name)) -> rm_data


###########################################################################

# Arrange ----
rm_data_for_arrange <- read_excel("C:/Users/SPoudel/Ventura Foods/SC Analytics Team - General/Stan Report Files/File Repository/IQR Automation/RM/Weekly Run Files/2025/02.11.2025/Raw Material Inventory Health (IQR) NEW TEMPLATE - 02.11.2025.xlsx",
                                  sheet = "RM data")
...
rm_data %>% 
  ...
  dplyr::select(mfg_loc, loc_name, item, loc_sku, supplier_number, supplier_name, description, class, item_type, shelf_life_day,
                ...
                opv, po_in_next_30_days, dos, dead_cost, lot_at_risk_cost, excess_cost, healthy_cycle_stock_cost, ss_oh_cost, upi_cost,
...
writexl::write_xlsx(rm_data, "C:/Users/SPoudel/Ventura Foods/SC Analytics Team - General/Stan Report Files/File Repository/IQR Automation/RM/Weekly Run Files/2025/02.11.2025/iqr_rm_rstudio_02042025.xlsx")





writexl::write_xlsx(bom, "C:/Users/SPoudel/Ventura Foods/SC Analytics Team - General/Stan Report Files/File Repository/IQR Automation/RM/Weekly Run Files/2025/02.11.2025/bom.xlsx")
...
file.copy("C:/Users/slee/OneDrive - Ventura Foods/Ventura Work/SCE/Project/FY 23/IQR Automation/FG/weekly run data/2025/02.04.2025/Inventory Health (IQR) Tracker - DOS.xlsx",
          "C:/Users/slee/OneDrive - Ventura Foods/Ventura Work/SCE/Project/FY 23/IQR Automation/RM/Weekly Report run/2025/02.11.2025/Inventory Health (IQR) Tracker - DOS.xlsx",
          overwrite = TRUE)
...
file.copy("C:/Users/SPoudel/Ventura Foods/SC Analytics Team - General/Stan Report Files/File Repository/IQR Automation/RM/Weekly Report run/2025/02.11.2025/Raw Material Inventory Health (IQR) NEW TEMPLATE - 02.11.2025.xlsx",
          "S:/Supply Chain Projects/LOGISTICS/SCP/Cost Saving Reporting/Inventory Days On Hand/Raw Material Inventory Health (IQR) NEW TEMPLATE - 02.11.2025.xlsx",
          overwrite = TRUE)

#### IQR main pre-week file Moving in S Drive. 
file.copy("S:/Supply Chain Projects/LOGISTICS/SCP/Cost Saving Reporting/Inventory Days On Hand/Raw Material Inventory Health (IQR) NEW TEMPLATE - 02.04.2025.xlsx",
          "S:/Supply Chain Projects/LOGISTICS/SCP/Cost Saving Reporting/Inventory Days On Hand/IQR Historical Data Collection/RM/Raw Material Inventory Health (IQR) NEW TEMPLATE - 02.11.2025.xlsx",
          overwrite = TRUE)
...
monthly_consumption <- read_excel("C:/Users/SPoudel/Ventura Foods/SC Analytics Team - General/Stan Report Files/File Repository/IQR Automation/RM/Weekly Run Files/2025/02.11.2025/Raw Material Monthly Consumption.xlsx")
...
monthly_consumption %>% 
  ...
  readr::type_convert() %>% 
  dplyr::mutate(base_product_code = as.numeric(base_product_code),
  ...
  dplyr::mutate(loc_sku = paste0(campus, "_", base_product_code)) %>% 
