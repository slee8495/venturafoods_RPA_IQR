...
exception_report_dnrr <- read_excel("S:/Supply Chain Projects/Data Source (SCE)/JDE DNRR Exception report extract/2024/exception report DOU 2024.10.15.xlsx")
...
inventory_rm <- read_excel("S:/Supply Chain Projects/Data Source (SCE)/Inventory/Inventory with Lot Report v.2 - 2024.10.15.xlsx",
                           sheet = "RM")
oo_bt_fg <- read_excel("S:/Supply Chain Projects/Data Source (SCE)/Report ingredients/Stan/10152024/US and CAN OO BT where status _ J.xlsx")
dsx <- read_excel("S:/Global Shared Folders/Large Documents/S&OP/Demand Planning/BI Forecast Backup/2024/DSX Forecast Backup - 2024.10.15.xlsx")
jde_25_55_label <- read_excel("S:/Supply Chain Projects/Data Source (SCE)/Inventory/JDE Inventory Lot Detail - 2024.10.15.xlsx")
lot_status_code <- read_excel("S:/Supply Chain Projects/Data Source (SCE)/Lot Status Code.xlsx")
bom <- read_excel("S:/Supply Chain Projects/Data Source (SCE)/Report ingredients/Stan/10152024/Bill of Material_101524.xlsx")
campus_ref <- read_excel("S:/Supply Chain Projects/Data Source (SCE)/Campus reference.xlsx")
complete_sku_list <- read_excel("S:/Supply Chain Projects/Data Source (SCE)/Report ingredients/Stan/10152024/Complete SKU list - Linda_2.xlsx")
...
unit_cost <- read_excel("S:/Supply Chain Projects/Data Source (SCE)/Report ingredients/Stan/10152024/Unit_Cost.xlsx")
...
final_data_fg %>% 
  ...
  dplyr::filter(str_detect(item, "^\\d{5}[A-Z]{3}$"))-> final_data_fg
...
final_data_fg %>% 
  dplyr::left_join(rbind(exception_report, exception_report_dnrr) %>% 
                     janitor::clean_names() %>% 
                     dplyr::select(b_p, item_number, description) %>% 
                     dplyr::mutate(ref = paste0(b_p, "_", item_number)) %>%
                     dplyr::distinct(ref, .keep_all = TRUE) %>% 
                     dplyr::select(ref, description), by = "ref") -> final_data_fg
...
writexl::write_xlsx(final_data_fg, "C:/Users/slee/OneDrive - Ventura Foods/Ventura Work/SCE/Project/FY 23/IQR Automation/FG/weekly run data/2024/10.15.2024/fg_optimization.xlsx")
