...
file.copy("C:/Users/SPoudel/Ventura Foods/SC Analytics Team - General/Stan Report Files/File Repository/IQR Automation/FG/Weekly Run Files/2025/02.04.2025/Finished Goods Inventory Health Adjusted Forward (IQR) NEW TEMPLATE - 02.04.2025.xlsx",
          "C:/Users/SPoudel/Ventura Foods/SC Analytics Team - General/Stan Report Files/File Repository/IQR Automation/FG/Weekly Run Files/2025/02.11.2025/Finished Goods Inventory Health Adjusted Forward (IQR) NEW TEMPLATE - 02.11.2025.xlsx",
          overwrite = TRUE)
...
# macro_platform ----
macro_platform <- read_excel("C:/Users/SPoudel/Ventura Foods/SC Analytics Team - General/Stan Report Files/File Repository/IQR Automation/FG/Weekly Run Files/2025/02.11.2025/Finished Goods Inventory Health Adjusted Forward (IQR) NEW TEMPLATE - 02.11.2025.xlsx",
...
receipt <- read_excel("C:/Users/SPoudel/Ventura Foods/SC Analytics Team - General/Stan Report Files/File Repository/Lookup Datasets/Weekly Dataset/02.11.2025/PO1.xlsx")
...
receipt %>% 
  janitor::clean_names() %>% 
  dplyr::mutate(po_requested_ship_date = as.double(po_requested_ship_date),
                po_requested_ship_date = as.Date(po_requested_ship_date, origin = "1899-12-30"),
                location = as.double(location),
                number_po_scheduled_cases = as.double(number_po_scheduled_cases)) %>% 
  dplyr::mutate(item = ifelse(str_detect(item, "^[0-9]{5}[A-Z]{3}$"), item, ifelse(str_detect(item, "^[0-9]+$"), as.character(as.numeric(item)), item))) %>% 
  dplyr::mutate(ref = paste0(location, "_", item)) %>% 
  dplyr::select(ref, po_requested_ship_date, number_po_scheduled_cases) %>% 
  dplyr::mutate(next_28_days = ifelse(po_requested_ship_date >= specific_date & po_requested_ship_date <= specific_date + 28, "Y", "N")) %>% 
  dplyr::rename(qty = number_po_scheduled_cases) -> receipt
...
wo %>% 
  ...
  dplyr::mutate(N = if ("N" %in% names(.)) as.integer(N) else as.integer(0)) -> wo_pivot
...
DSX_Forecast_Backup <- read_excel(
  "S:/Global Shared Folders/Large Documents/S&OP/Demand Planning/BI Forecast Backup/2025/DSX Forecast Backup - 2025.01.23.xlsx") ### Match with BoM date! ####
...
inventory <- read_excel("C:/Users/SPoudel/Ventura Foods/SC Analytics Team - General/Stan Report Files/File Repository/Lookup Datasets/Weekly Dataset/02.11.2025/Inventory with Lot Report v.2 - 2025.02.11.xlsx",
                        sheet = "FG")
...
IQR_FG_sample <- read_excel("C:/Users/SPoudel/Ventura Foods/SC Analytics Team - General/Stan Report Files/File Repository/IQR Automation/FG/Weekly Run Files/2025/02.11.2025/Finished Goods Inventory Health Adjusted Forward (IQR) NEW TEMPLATE - 02.11.2025.xlsx",
                            sheet = "Location FG")
...
sdcv <- read_excel("C:/Users/SPoudel/Ventura Foods/SC Analytics Team - General/Stan Report Files/File Repository/Lookup Datasets/Weekly Dataset/02.11.2025/Ordered and Shipped History (Ordered) - Month - 02.11.2025.xlsx")
...
IQR_FG_sample <- IQR_FG_sample %>%
  left_join(select(exception_report, ref, Order_Policy_Code), by = "ref") %>%
  mutate(opv = as.integer(Order_Policy_Code)) %>%
...
merge(IQR_FG_sample, custord_mfg_pivot_1[, c("ref", "Y")], by = "ref", all.x = TRUE) %>% 
  dplyr::mutate(y_na = !is.na(Y)) %>% 
...
merge(IQR_FG_sample, custord_mfg_pivot_2[, c("ref", "Y")], by = "ref", all.x = TRUE) %>% 
  dplyr::mutate(y_na = !is.na(Y)) %>% 
...
merge(IQR_FG_sample, custord_mfg_pivot_3[, c("ref", "Y")], by = "ref", all.x = TRUE) %>% 
  dplyr::mutate(y_na = !is.na(Y)) %>% 
...
merge(IQR_FG_sample, custord_mfg_pivot_4[, c("ref", "Y")], by = "ref", all.x = TRUE) %>% 
  dplyr::mutate(y_na = !is.na(Y)) %>% 
...
# pre_data
pre_data <- read_excel("C:/Users/SPoudel/Ventura Foods/SC Analytics Team - General/Stan Report Files/File Repository/IQR Automation/FG/Weekly Run Files/2025/02.04.2025/iqr_fg_rstudio_01282025.xlsx")
...
completed_sku_list_2 <- read_excel("C:/Users/SPoudel/Ventura Foods/SC Analytics Team - General/Stan Report Files/File Repository/Lookup Datasets/Weekly Dataset/02.11.2025/Complete SKU list - Linda.xlsx")
...
exception_report_dou <- read_excel("C:/Users/SPoudel/Ventura Foods/SC Analytics Team - General/Stan Report Files/File Repository/Lookup Datasets/Weekly Dataset/02.11.2025/exception report DOU 2025.02.11.xlsx")
...
IQR_FG_sample %>% 
  dplyr::mutate(current_ss_plt = "NA",
                max_cycle_stock_plt = "NA",
                avg_cycle_stock_plt = "NA",
                quality_hold_plt = "NA",
                on_hand_plt = "NA",
                total_inventory_plt = "NA",
                inventory_target_plt = "NA",
                max_inventory_target_plt = "NA",
                current_xs_pallets = "NA") -> IQR_FG_sample


################### Added on 10/08/2025

IQR_FG_sample %>% 
  dplyr::mutate(total_inventory = "NA",
                total_inventory_lbs = "NA",
                total_inventory_dollar = "NA",
                current_ss_plt = "NA",
                max_mfg_cycle_stock = "NA", 
                max_mfg_cycle_stock_dollar = "NA", 
                max_mfg_cycle_stock_plt = "NA", 
                avg_mfg_cycle_stock_plt = "NA",
                inventory_target_with_upi_target = "NA",
                inventory_target_with_upi_target_lbs = "NA", 
...
IQR_FG_sample %>% 
  ...
  dplyr::mutate(planner = ifelse(planner == "DNRR", 0, planner),
                planner_name = ifelse(planner_name == "DNRR", 0, planner_name)) -> IQR_FG_sample
...
# Arrange ----
fg_data_for_arrange <- read_excel("C:/Users/SPoudel/Ventura Foods/SC Analytics Team - General/Stan Report Files/File Repository/IQR Automation/FG/Weekly Run Files/2025/02.11.2025/Finished Goods Inventory Health Adjusted Forward (IQR) NEW TEMPLATE - 02.11.2025.xlsx",
                                  sheet = "Location FG")
...
IQR_FG_sample %>% 
  ...
  dplyr::select(loc, mfg_loc, campus, item_2, category, platform, macro_platform, sub_type, cvm, focus_label, ref, 
                ...
                on_hand_inv_after_mfg_28_days_cust_ord_0, current_month_fcst_2, next_month_fcst_2, campus_weighted_cost) -> IQR_FG_sample



# (Path Revision Needed)
writexl::write_xlsx(IQR_FG_sample, "C:/Users/SPoudel/Ventura Foods/SC Analytics Team - General/Stan Report Files/File Repository/IQR Automation/FG/Weekly Run Files/2025/02.11.2025/iqr_fg_rstudio_02042025.xlsx")
...
file.copy("C:/Users/SPoudel/Ventura Foods/SC Analytics Team - General/Stan Report Files/File Repository/IQR Automation/RM/Weekly Report run/2025/02.11.2025/Inventory Health (IQR) Tracker - DOS.xlsx",
          "C:/Users/SPoudel/Ventura Foods/SC Analytics Team - General/Stan Report Files/File Repository/IQR Automation/FG/weekly run data/2025/02.11.2025/Inventory Health (IQR) Tracker - DOS.xlsx")


### Optional ###
#### IQR main file Moving to S Drive. 
file.copy("C:/Users/SPoudel/Ventura Foods/SC Analytics Team - General/Stan Report Files/File Repository/IQR Automation/FG/Weekly Run Files/2025/02.11.2025/Finished Goods Inventory Health Adjusted Forward (IQR) NEW TEMPLATE - 02.11.2025.xlsx",
          "S:/Supply Chain Projects/LOGISTICS/SCP/Cost Saving Reporting/Inventory Days On Hand/Finished Goods Inventory Health Adjusted Forward (IQR) NEW TEMPLATE - 02.11.2025.xlsx",
          overwrite = TRUE)



### DOS Tracker moving to S Drive
file.copy("C:/Users/SPoudel/Ventura Foods/SC Analytics Team - General/Stan Report Files/File Repository/IQR Automation/FG/Weekly Run Files/2025/02.11.2025/Inventory Health (IQR) Tracker - DOS.xlsx",
...
file.copy("S:/Supply Chain Projects/LOGISTICS/SCP/Cost Saving Reporting/Inventory Days On Hand/Finished Goods Inventory Health Adjusted Forward (IQR) NEW TEMPLATE - 02.04.2025.xlsx",
          "S:/Supply Chain Projects/LOGISTICS/SCP/Cost Saving Reporting/Inventory Days On Hand/IQR Historical Data Collection/FG/2025/Finished Goods Inventory Health Adjusted Forward (IQR) NEW TEMPLATE - 02.04.2025.xlsx",
          overwrite = TRUE)
...
pre_month_actual <- read_excel("C:/Users/SPoudel/Ventura Foods/SC Analytics Team - General/Stan Report Files/File Repository/IQR Automation/FG/Weekly Run Files/2025/02.11.2025/Actual Shipped.xlsx")
