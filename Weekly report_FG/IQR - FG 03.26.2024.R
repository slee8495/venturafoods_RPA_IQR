...
# macro_platform ----
macro_platform <- read_excel("C:/Users/slee/OneDrive - Ventura Foods/Ventura Work/SCE/Project/FY 23/IQR Automation/FG/weekly run data/2024/03.26.2024/Finished Goods Inventory Health Adjusted Forward (IQR) NEW TEMPLATE - 03.26.2024.xlsx",
...
custord %>% 
  ...
  dplyr::mutate(ref = paste0(Location, "_", Item),
                in_next_7_days = ifelse(date < specific_date + 7, "Y", "N"),
                in_next_14_days = ifelse(date < specific_date + 14, "Y", "N"),
                in_next_21_days = ifelse(date < specific_date + 21, "Y", "N"),
                in_next_28_days = ifelse(date < specific_date + 28, "Y", "N")) %>% 
  dplyr::relocate(ref, Item, Location) -> custord


# (Path Revision Needed) Loc 624 for custord ----
loc_624_custord <- read_excel("C:/Users/slee/OneDrive - Ventura Foods/Ventura Work/SCE/Project/FY 23/BoM version 2/Weekly Run/2024/03.26.2024/Canada Open Orders.xlsx", 
                              col_names = FALSE)
...
loc_624_custord %>% 
  ...
  dplyr::mutate(date = as.integer(date),
                ...
                Qty = as.double(Qty)) %>% 
  dplyr::mutate(in_next_7_days = ifelse(date < specific_date + 7, "Y", "N"),
                in_next_14_days = ifelse(date < specific_date + 14, "Y", "N"),
                in_next_21_days = ifelse(date < specific_date + 21, "Y", "N"),
                in_next_28_days = ifelse(date < specific_date + 28, "Y", "N")) %>% 
...
loc_39_bt %>% 
  janitor::clean_names() %>% 
  readr::type_convert() %>% 
  dplyr::rename(date = sales_order_requested_ship_date) %>% 
  mutate(date = as.Date(date, origin = "1899-12-30")) %>% 
  dplyr::select(location, product_label_sku, b_t_open_order_cases, date) %>% 
  dplyr::mutate(product_label_sku = gsub("-", "", product_label_sku),
                ref = paste0(location, "_", product_label_sku)) %>% 
  dplyr::filter(location == 39) %>% 
  
  dplyr::group_by(ref) %>% 
  dplyr::summarise(bt_sum = sum(b_t_open_order_cases),
                   date = first(date)) %>% 
  dplyr::ungroup() %>% 
  ...
  dplyr::mutate(in_next_7_days = ifelse(date < specific_date + 7, "Y", "N"),
                in_next_14_days = ifelse(date < specific_date + 14, "Y", "N"),
                in_next_21_days = ifelse(date < specific_date + 21, "Y", "N"),
                in_next_28_days = ifelse(date < specific_date + 28, "Y", "N")) -> loc_39_bt_2


loc_39_bt_2 %>% 
  dplyr::mutate(ref_2 = ref) %>% 
  tidyr::separate(ref_2, c("Location", "Item"), sep = "_") %>% 
...
DSX_Forecast_Backup_pre <- read_excel(
  "S:/Global Shared Folders/Large Documents/S&OP/Demand Planning/BI Forecast Backup/2024/DSX Forecast Backup - 2024.02.01.xlsx")
...
DSX_Forecast_Backup <- read_excel(
  "S:/Global Shared Folders/Large Documents/S&OP/Demand Planning/BI Forecast Backup/2024/DSX Forecast Backup - 2024.03.25.xlsx") ### Match with BoM date! ####
...
inventory <- read_excel("S:/Supply Chain Projects/Data Source (SCE)/Inventory/Inventory with Lot Report v.2 - 2024.03.26.xlsx",
                        sheet = "FG")
...
IQR_FG_sample <- read_excel("C:/Users/slee/OneDrive - Ventura Foods/Ventura Work/SCE/Project/FY 23/IQR Automation/FG/weekly run data/2024/03.26.2024/Finished Goods Inventory Health Adjusted Forward (IQR) NEW TEMPLATE - 03.26.2024.xlsx",
                            sheet = "Location FG")
...
sdcv <- read_excel("S:/Supply Chain Projects/LOGISTICS/SCP/Cost Saving Reporting/Standard Deviation & CV/Standard Deviation, CV,  March 2024.xlsx")
...
######## added 7/5/2023 #########
# wo_2
wo_2 <- read.csv("C:/Users/slee/OneDrive - Ventura Foods/Ventura Work/SCE/Project/FY 23/DSXIE/2024/03.26/wo.csv",
                 header = FALSE)
...
# receipt_2
receipt_2 <- read.csv("C:/Users/slee/OneDrive - Ventura Foods/Ventura Work/SCE/Project/FY 23/DSXIE/2024/03.26/receipt.csv",
                      header = FALSE)
...
receipt_2 %>%   
  ...
  dplyr::left_join(Campus_ref %>% mutate(Campus = as.character(Campus),
                                         B_P = as.character(B_P)) %>% select(B_P, Campus) %>% rename(Location = B_P)) %>% 
...
# pre_data
pre_data <- read_excel("C:/Users/slee/OneDrive - Ventura Foods/Ventura Work/SCE/Project/FY 23/IQR Automation/FG/weekly run data/2024/03.19.2024/iqr_fg_rstudio_03192024.xlsx")
...
# Arrange ----
fg_data_for_arrange <- read_excel("C:/Users/slee/OneDrive - Ventura Foods/Ventura Work/SCE/Project/FY 23/IQR Automation/FG/weekly run data/2024/03.26.2024/Finished Goods Inventory Health Adjusted Forward (IQR) NEW TEMPLATE - 03.26.2024.xlsx",
                                  sheet = "Location FG")
...
writexl::write_xlsx(IQR_FG_sample, "C:/Users/slee/OneDrive - Ventura Foods/Ventura Work/SCE/Project/FY 23/IQR Automation/FG/weekly run data/2024/03.26.2024/iqr_fg_rstudio_03262024.xlsx")
writexl::write_xlsx(custord_open_order, "C:/Users/slee/OneDrive - Ventura Foods/Ventura Work/SCE/Project/FY 23/IQR Automation/FG/weekly run data/2024/03.26.2024/Open Order.xlsx")
...
file.copy("C:/Users/slee/OneDrive - Ventura Foods/Ventura Work/SCE/Project/FY 23/IQR Automation/RM/Weekly Report run/2024/03.26.2024/Inventory Health (IQR) Tracker - DOS.xlsx",
          "C:/Users/slee/OneDrive - Ventura Foods/Ventura Work/SCE/Project/FY 23/IQR Automation/FG/weekly run data/2024/03.26.2024/Inventory Health (IQR) Tracker - DOS.xlsx")



#### IQR main file Moving to S Drive. 
file.copy("C:/Users/slee/OneDrive - Ventura Foods/Ventura Work/SCE/Project/FY 23/IQR Automation/FG/weekly run data/2024/03.26.2024/Finished Goods Inventory Health Adjusted Forward (IQR) NEW TEMPLATE - 03.26.2024.xlsx",
          "S:/Supply Chain Projects/LOGISTICS/SCP/Cost Saving Reporting/Inventory Days On Hand/Finished Goods Inventory Health Adjusted Forward (IQR) NEW TEMPLATE - 03.26.2024.xlsx")



### DOS Tracker moving to S Drive
file.copy("C:/Users/slee/OneDrive - Ventura Foods/Ventura Work/SCE/Project/FY 23/IQR Automation/FG/weekly run data/2024/03.26.2024/Inventory Health (IQR) Tracker - DOS.xlsx",
...
file.copy("S:/Supply Chain Projects/LOGISTICS/SCP/Cost Saving Reporting/Inventory Days On Hand/Finished Goods Inventory Health Adjusted Forward (IQR) NEW TEMPLATE - 03.19.2024.xlsx",
          "S:/Supply Chain Projects/LOGISTICS/SCP/Cost Saving Reporting/Inventory Days On Hand/IQR Historical Data Collection/FG/2024/Finished Goods Inventory Health Adjusted Forward (IQR) NEW TEMPLATE - 03.19.2024.xlsx",
          overwrite = TRUE)
...
pre_month_actual <- read_excel("C:/Users/slee/OneDrive - Ventura Foods/Ventura Work/SCE/Project/FY 23/IQR Automation/FG/weekly run data/2024/03.05.2024/Actual Shipped.xlsx")
