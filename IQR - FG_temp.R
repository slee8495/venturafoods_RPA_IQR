...
# Planner Address Book (If updated, correct this link)  # S Drive ----
...
# Exception Report  # sheet = "Sheet1 (2)" ----

exception_report <- read_excel("C:/Users/SLee/OneDrive - Ventura Foods/Ventura Work/SCE/Project/FY 23/IQR Automation/FG/FG test 3/exception report 06.22.22.xlsx", 
                               sheet = "Sheet1 (2)",
...
Campus_ref <- read_excel("C:/Users/SLee/OneDrive - Ventura Foods/Ventura Work/SCE/Project/FY 23/IQR Automation/RM/campus reference.xlsx", 
...
# Custord PO ----
po <- read_excel("C:/Users/SLee/OneDrive - Ventura Foods/Ventura Work/SCE/Project/FY 23/IQR Automation/FG/FG test 3/wo receipt custord po - 06.22.22.xlsx", 
...
po %>% 
  ...
  reshape2::dcast(ref ~ next_28_days, value.var = "Qty", sum) -> PO_Pivot


# Custord Receipt ----
receipt <- read_excel("C:/Users/SLee/OneDrive - Ventura Foods/Ventura Work/SCE/Project/FY 23/IQR Automation/FG/FG test 3/wo receipt custord po - 06.22.22.xlsx", 
...
receipt %>% 
  ...
  dplyr::filter(label_na == TRUE) %>%
  tidyr::separate(aa, c("global", "rp", "Item")) %>% 
  dplyr::select(-global, -rp, -label, -label_na, -"4") %>% 
  dplyr::rename(Location = "2",
                Qty = "5",
                po_no = "6",
                date = "7") %>% 
...
receipt %>% 
  dplyr::mutate(date = as.Date(date)) %>% 
  dplyr::mutate(next_28_days = ifelse(date <= Sys.Date() + 28, "Y", "N")) %>% 
  reshape2::dcast(ref ~ next_28_days, value.var = "Qty", sum) -> Receipt_Pivot  
...
# Custord wo ----
wo <- read_excel("C:/Users/SLee/OneDrive - Ventura Foods/Ventura Work/SCE/Project/FY 23/IQR Automation/FG/FG test 3/wo receipt custord po - 06.22.22.xlsx", 
...
substr(wo$`1`, nchar(wo$`1`)-2, nchar(wo$`1`)) -> label_wo


wo %>% 
  dplyr::bind_cols(label_wo) %>% 
  dplyr::rename(label = "...8",
                aa = "1") %>% 
  dplyr::mutate(label_na = as.integer(label),
                label_na = is.na(label_na)) %>% 
  ...
  dplyr::mutate(Location = sub("^0+", "", Location)) %>% 
  dplyr::mutate(ref = paste0(Location, "_", Item)) %>% 
  dplyr::relocate(ref, Item, Location) -> wo
...
custord <- read_excel("C:/Users/SLee/OneDrive - Ventura Foods/Ventura Work/SCE/Project/FY 23/IQR Automation/FG/FG test 3/wo receipt custord po - 06.22.22.xlsx", 
...
# DSX Forecast pulling (Previous month file)---- Change Directory ----

DSX_Forecast_Backup_pre <- read_excel(
  "S:/Global Shared Folders/Large Documents/S&OP/Demand Planning/Demand Planning Team/BI Forecast Backup/2022/DSX Forecast Backup - 2022.05.02.xlsx")
...
DSX_Forecast_Backup <- read_excel(
  "S:/Global Shared Folders/Large Documents/S&OP/Demand Planning/Demand Planning Team/BI Forecast Backup/DSX Forecast Backup - 2022.06.21.xlsx")
...
# Read FG ----
Inventory_analysis_FG <- read_excel("C:/Users/SLee/OneDrive - Ventura Foods/Ventura Work/SCE/Project/FY 23/IQR Automation/FG/FG test 3/Inventory Report for all locations - 06.22.22.xlsx", 
                                    sheet = "FG")
...
IQR_FG_sample <- read_excel("C:/Users/SLee/OneDrive - Ventura Foods/Ventura Work/SCE/Project/FY 23/IQR Automation/FG/FG test 3/Finished Goods Inventory Health Adjusted Forward (IQR) - 06.22.22.xlsx",
...
# read SD & CV file ----
sdcv <- read_excel("S:/Supply Chain Projects/LOGISTICS/SCP/Cost Saving Reporting/Standard Deviation, CV,  May 2022 - 05.04.22.xlsx")
...
# calculate - Forward Inv Target Current Month Fcst (Business days input - ## Current Month ##) # make sure with business day (either 20, 21, 22 usually) ---- 
IQR_FG_sample %<>% 
  dplyr::mutate(Forward_Inv_Target_Current_Month_Fcst = (pmax((Current_Month_Fcst / 22) * OPV, JDE_MOQ)) / 2 + Current_SS,
...
# calculate - on hand Inv <= mfg AF max
IQR_FG_sample %>% 
  dplyr::mutate(on_hand_Inv_less_or_equal_mfg_AF_max = "=IF(AJ2<=BA2,1,0)") -> IQR_FG_sample
...
IQR_FG_sample %>% 
  dplyr::mutate(on_hand_Inv_less_or_equal_mfg_AF_target = "=IF(AJ2<=AX2,1,0)") -> IQR_FG_sample


# calculate - on hand Inv after CustOrd <= mfg AF max
IQR_FG_sample %>% 
  dplyr::mutate(on_hand_Inv_after_CustOrd_less_or_equal_mfg_AF_max =  
                  "=IF(AJ2-(IF(OR(BG2=0,BG2>=20),BP2,IF(AND(BG2<20,BG2>=12),BO2,IF(AND(BG2<12,BG2>=8),BN2,BM2))))<=BA2,1,0)") -> IQR_FG_sample
...
IQR_FG_sample %>% 
  dplyr::mutate(on_hand_Inv_after_CustOrd_greater_mfg_AF_target = 
                  "=IF(AJ2-(IF(OR(BG2=0,BG2>=20),BP2,IF(AND(BG2<20,BG2>=12),BO2,IF(AND(BG2<12,BG2>=8),BN2,BM2))))>AX2,1,0)") -> IQR_FG_sample
...
IQR_FG_sample %>% 
  dplyr::mutate(on_hand_Inv_after_CustOrd_less_or_equal_mfg_AF_target = 
                  "=IF(AJ2-(IF(OR(BG2=0,BG2>=20),BP2,IF(AND(BG2<20,BG2>=12),BO2,IF(AND(BG2<12,BG2>=8),BN2,BM2))))<=AX2,1,0)") ->
  IQR_FG_sample


# calculate - on hand inv after mfg 28 days CustOrd > 0
IQR_FG_sample %>%
  dplyr::mutate(on_hand_inv_after_mfg_28_days_CustOrd_greater_0 = "=IF(AJ2-BP2>0,1,0)") -> IQR_FG_sample
...
IQR_FG_sample %<>% 
  ...
  dplyr::relocate(Loc, mfg_loc, Campus, Item_2, category, Platform, Macro_Platform, Sub_Type, On_Priority_list, ref, 
                  ...
                  on_hand_Inv_greater_mfg_Adjusted_Forward_looking_target, on_hand_Inv_less_or_equal_AF_target,
...
# 1. formula change the row number from 5 to 2
# 2. dos.mfg.na error (Mfg_DOS_after_CustOrd, Mfg_DOS)
...
# 
# # for test
# IQR_FG_original <- read_excel("C:/Users/SLee/OneDrive - Ventura Foods/Ventura Work/SCE/Project/FY 23/IQR Automation/FG/FG test 3/Finished Goods Inventory Health Adjusted Forward (IQR) - 06.22.22.xlsx",
#                             sheet = "FG without BKO BKM TST")
# 
# IQR_FG_original[-1:-2,] -> IQR_FG_original
# 
# colnames(IQR_FG_original) <- IQR_FG_original[1, ]
# IQR_FG_original[-1, ] -> IQR_FG_original
# 
# names(IQR_FG_original) <- str_replace_all(names(IQR_FG_original), c(" " = "_"))
# names(IQR_FG_original) <- str_replace_all(names(IQR_FG_original), c("-" = "_"))
# names(IQR_FG_original) <- str_replace_all(names(IQR_FG_original), c("/" = "_"))
# names(IQR_FG_original) <- str_replace_all(names(IQR_FG_original), c(">" = "greater"))
# names(IQR_FG_original) <- str_replace_all(names(IQR_FG_original), c("<=" = "less_or_equal"))
# 
# 
# colnames(IQR_FG_original)[9] <- "On_Priority_list"
# colnames(IQR_FG_original)[32] <- "Useable"
# colnames(IQR_FG_original)[34] <- "Quality_hold_in_cost"
# colnames(IQR_FG_original)[36] <- "On_Hand_usable_and_soft_hold"
# colnames(IQR_FG_original)[38] <- "On_Hand_in_cost"
# colnames(IQR_FG_original)[39] <- "On_Hand_Adjusted_Forward_Max_in_cost"
# colnames(IQR_FG_original)[40] <- "On_Hand_Mfg_Adjusted_Forward_Max_in_cost"
# colnames(IQR_FG_original)[41] <- "Forward_Inv_Target_Current_Month_Fcst"
# colnames(IQR_FG_original)[43] <- "Forward_Inv_Target_Current_Month_Fcst_in_cost"
# colnames(IQR_FG_original)[46] <- "Adjusted_Forward_Inv_Target_in_cost"
# colnames(IQR_FG_original)[49] <- "Adjusted_Forward_Inv_Max_in_cost"
# colnames(IQR_FG_original)[52] <- "Mfg_Adjusted_Forward_Inv_Target_in_cost"
# colnames(IQR_FG_original)[55] <- "Mfg_Adjusted_Forward_Inv_Max_in_cost"
# colnames(IQR_FG_original)[56] <- "Forward_Inv_Target_lag_1_Current_Month_Fcst"
# colnames(IQR_FG_original)[58] <- "Forward_Inv_Target_lag_1_Current_Month_Fcst_in_cost"
# colnames(IQR_FG_original)[64] <- "CustOrd_in_next_28_days_in_cost"
# colnames(IQR_FG_original)[69] <- "Mfg_CustOrd_in_next_28_days_in_cost"
# colnames(IQR_FG_original)[75] <- "Forward_Target_Inv_DOS_fcst_only"
# colnames(IQR_FG_original)[76] <- "Adjusted_Forward_Target_Inv_DOS_includes_Orders"
# colnames(IQR_FG_original)[80] <- "Mfg_Forward_Target_Inv_DOS_fcst_only"
# colnames(IQR_FG_original)[81] <- "Mfg_Adjusted_Forward_Target_Inv_DOS_includes_Orders"
# colnames(IQR_FG_original)[85] <- "Lag_1_Current_Month_Fcst_in_cost"
# colnames(IQR_FG_original)[94] <- "has_adjusted_forward_looking_Max"
# colnames(IQR_FG_original)[104] <- "has_mfg_adjusted_forward_looking_Max"
# 
# 
# IQR_FG_original %<>% 
#   dplyr::mutate(Ref = gsub("-", "_", Ref),
...
#                 mfg_ref = Mfg_Ref)
# 
# readr::type_convert(IQR_FG_original) -> IQR_FG_original
# 
# 
# 
# skim(IQR_FG_sample) -> a
# skim(IQR_FG_original) -> b
# 
# writexl::write_xlsx(a, "sample_2.xlsx")
# writexl::write_xlsx(b, "original.xlsx")
# 
# 
